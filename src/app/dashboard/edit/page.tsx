...
export default function EditProfilePage() {
  ...
  const inputClass = "bg-[#111d2e] border border-[#ff4060]/30 rounded-lg px-3 py-2 font-mono text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#ff4060] w-full";

  return (
    <div className="min-h-screen bg-[#080c14] text-white p-6">
      <div className="max-w-2xl mx-auto">
        <div className="mb-8">
          <button onClick={() => router.back()} className="font-mono text-xs text-[#00c896] hover:underline mb-4 inline-block">
            ← Retour
          </button>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <input className={inputClass} value={form.headline} onChange={(e) => update("headline", e.target.value)} placeholder="Penetration Tester | OSCP" />
          </div>

          <div>
            <label className="font-mono text-xs text-gray-400 mb-1 block">Bio</label>
            <textarea className={`${inputClass} min-h-[80px]`} value={form.bio} onChange={(e) => update("bio", e.target.value)} placeholder="Parle de ton parcours..." />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="font-mono text-xs text-gray-400 mb-1 block">Localisation</label>
              <input className={inputClass} value={form.location} onChange={(e) => update("location", e.target.value)} placeholder="Dakar" />
            </div>
            <div>
              <label className="font-mono text-xs text-gray-400 mb-1 block">Pays</label>
              <input className={inputClass} value={form.country} onChange={(e) => update("country", e.target.value)} placeholder="SN" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input className={inputClass} value={form.githubUsername} onChange={(e) => update("githubUsername", e.target.value)} placeholder="username" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className={inputClass} value={form.twitterUrl} onChange={(e) => update("twitterUrl", e.target.value)} placeholder="https://x.com/..." />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input className={inputClass} value={form.phoneNumber} onChange={(e) => update("phoneNumber", e.target.value)} placeholder="+221..." />
            </div>
            <div>
              <label className="font-mono text-xs text-gray-400 mb-1 block">Timezone</label>
              <input className={inputClass} value={form.timezone} onChange={(e) => update("timezone", e.target.value)} placeholder="Africa/Dakar" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" className={inputClass} value={form.salaryMin} onChange={(e) => update("salaryMin", e.target.value)} placeholder="2000" />
            </div>
            <div>
              <label className="font-mono text-xs text-gray-400 mb-1 block">Salaire max (USD/mois)</label>
              <input type="number" className={inputClass} value={form.salaryMax} onChange={(e) => update("salaryMax", e.target.value)} placeholder="5000" />
            </div>
          </div>

          <div className="flex items-center gap-3">
            <input type="checkbox" id="available" checked={form.isAvailable} onChange={(e) => update("isAvailable", e.target.checked)} className="accent-[#00c896]" />
            <label htmlFor="available" className="font-mono text-sm text-gray-300">Disponible pour des opportunités</label>
          </div>
          ...
          <div className="flex items-center gap-4 pt-4">
            ...
            {message && (
              <span className="font-mono text-xs text-gray-400">{message}</span>
            )}
          </div>
        </form>
      </div>
