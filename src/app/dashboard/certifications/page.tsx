  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="font-mono text-xs text-gray-400">{certs.length} certification(s) ajoutée(s)</p>
      </div>

      {certs.length > 0 && (
        <div className="flex flex-col gap-3 mb-8">
          {certs.map((cert) => (
            <div key={cert.id} className="bg-[#0d1520] border border-[#00c896]/20 rounded-xl p-5 flex items-center justify-between">
              <div>
                ...
                <div className="font-mono text-xs text-gray-500 mt-1">{cert.issuer} · {new Date(cert.issuedAt).toLocaleDateString("fr-FR")}</div>
                {cert.credentialUrl && (
                  <a href={cert.credentialUrl} target="_blank" className="font-mono text-xs text-[#00c896] hover:underline">
                    Vérifier →
                  </a>
                )}
              </div>
