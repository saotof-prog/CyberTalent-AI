...
export function detectCertificatePlatform(url: string): PlatformDetectionResult {
  ...
  try {
    ...
    if (hostname.includes('coursera.org')) {
      return { platform: 'coursera', confidence: 0.9, platformSpecificData: { platform: 'coursera' } };
    }
    ...
    const certPatterns = [/certificate/i, /badge/i, /verify/i, /credential/i, /accomplishment/i, /diploma/i];
...
export function isLikelyCertificateUrl(url: string): {
  ...
  if (detection.platform !== 'unknown' && detection.confidence >= 0.6) {
    return { isLikely: true, confidence: detection.confidence, reasons: [`Plateforme détectée : ${detection.platform}`] };
  }
